...
describe('VTimePickerTitle.ts', () => {
  ...
  it('should emit event when clicking hour, minute, second', async () => {
    ...
    expect(selectMode).toHaveBeenCalledWith(SelectMode.Hour)

    buttons.at(1).element.click()
    await wrapper.vm.$nextTick()

    expect(selectMode).toHaveBeenCalledWith(SelectMode.Minute)

    buttons.at(2).element.click()
    await wrapper.vm.$nextTick()

    expect(selectMode).toHaveBeenCalledWith(SelectMode.Second)
  })
